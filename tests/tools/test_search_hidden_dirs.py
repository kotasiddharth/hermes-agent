...
@pytest.mark.skipif(os.name == "nt", reason="requires POSIX find")
...
@pytest.mark.skipif(os.name == "nt", reason="requires POSIX grep")
class TestGrepExcludesHiddenDirs:
    ...
    def test_grep_skips_hub_cache(self, searchable_tree):
        ...
        cmd = f"grep -rnHE --exclude-dir='.*' 'ignore' {searchable_tree}"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    ...
    def test_grep_still_finds_visible_content(self, searchable_tree):
        ...
        cmd = f"grep -rnHE --exclude-dir='.*' 'real skill' {searchable_tree}"
        result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
        assert "SKILL.md" in result.stdout
